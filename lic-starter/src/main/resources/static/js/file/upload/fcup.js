...
(function (exports) {
    let fcup = function (config) {
        ...
        this.checkurl = '';
        this.url = '';
        this.headers = {"version": "fcup-v2.1"};
        this.type = '';
        this.apped_data = {};
        this.currentsize = 0;
        this.currenttime = 0;
        ...
        this.errormsg = {1000: "未找到该上传id", 1001: "不允许上传的文件类型", 1002: "上传文件过小", 1003: "上传文件过大", 1004: "请求超时"};
        this.start = function () {
        };
        ...
        this.upcheck = function (md5) {
            ...
            this.formdata.append('size', that.size);
            this.formdata.append('total', that.shardTotal);
            if (this.apped_data) {
                this.formdata.append("apped_data", JSON.stringify(this.apped_data));
            }
            let xhrs = new XMLHttpRequest();
            let header = this.headers;
            xhrs.open('POST', this.checkurl, false);
            if (this.headers) {
            ...
            this.formdata.delete('md5');
            this.formdata.delete('name');
            this.formdata.delete('size');
            this.formdata.delete('total');
            if (that.apped_data) {
        ...
        this.post = function (md5) {
            ...
            this.formdata.append("data", file['data']);
            this.formdata.append("name", file['name']);
            this.formdata.append("size", file['size'] );
            this.formdata.append("chunksize", file['chunksize'] );
            this.formdata.append("suffix", file['suffix']);
            this.formdata.append("total", shardCount);
            this.formdata.append("md5", md5);
            this.formdata.append("index", this.shardIndex);
            this.currentsize += file['chunksize'];
            if (this.apped_data) {
                this.formdata.append("apped_data", JSON.stringify(this.apped_data));
            }
            let xhrs = this.xhr;
            ...
            this.formdata.delete('name');
            this.formdata.delete('size');
            this.formdata.delete("chunksize");
            this.formdata.delete("suffix");
            this.formdata.delete('md5');
            this.formdata.delete('total');
        ...
        this.postData = function (i, start, end) {
            ...
            this.datas[i]["data"] = file;
            this.datas[i]["name"] = this.filename;
            this.datas[i]["size"] = this.size ;
            this.datas[i]["chunksize"] = file.size ;
            this.datas[i]["suffix"] = this.suffix;
        };
        this.limitFileSize = function (limitSize) {
            var arr = ["KB", "MB", "GB"], limit = limitSize.toUpperCase(), limitNum = 0;
        ...
        this.upload = function (file) {
            ...
            let ext = this.filename.lastIndexOf("."), ext_len = this.filename.length;
            ...
            let i = 0, blobSlice = File.prototype.slice || File.prototype.mozSlice || File.prototype.webkitSlice,
                chunkSize = (+this.shardsize) * 1024 * 1024, chunks = Math.ceil(file.size / chunkSize),
                currentChunk = 0, md5id = 0, fileReader = new FileReader(), spark = new SparkMD5.ArrayBuffer();
